...
void InitAddressMap()
{
	...
	// Print current allocation policy
	xil_printf("\r\n=== ALLOCATION POLICY INFO ===\r\n");
#if defined(ALLOCATION_POLICY_1) || !defined(ALLOCATION_POLICY_2) && !defined(ALLOCATION_POLICY_3) && !defined(ALLOCATION_POLICY_4)
	xil_printf("Policy: Channel -> Way -> Page -> Block (Policy 1)\r\n");
#elif defined(ALLOCATION_POLICY_2)
	xil_printf("Policy: Way -> Channel -> Block -> Page (Policy 2)\r\n");
#elif defined(ALLOCATION_POLICY_3)
	xil_printf("Policy: Block -> Page -> Way -> Channel (Policy 3)\r\n");
#elif defined(ALLOCATION_POLICY_4)
	xil_printf("Policy: Page -> Block -> Channel -> Way (Policy 4)\r\n");
#endif
	xil_printf("USER_CHANNELS=%d, USER_WAYS=%d, USER_DIES=%d\r\n", USER_CHANNELS, USER_WAYS, USER_DIES);
	xil_printf("=============================\r\n");
...
#if defined(ALLOCATION_POLICY_1) || !defined(ALLOCATION_POLICY_2) && !defined(ALLOCATION_POLICY_3) && !defined(ALLOCATION_POLICY_4)
	// Policy 1: Channel → Way → Page → Block (기본값)
...
#elif defined(ALLOCATION_POLICY_2)
	// Policy 2: Way → Channel → Block → Page
	targetDie = Pcw2VdieTranslation(targetCh, targetWay);
	
	if(targetWay != (USER_WAYS - 1))
		targetWay = targetWay + 1;
	else
	{
		targetWay = 0;
		targetCh = (targetCh + 1) % USER_CHANNELS;
	}

#elif defined(ALLOCATION_POLICY_3)
	// Policy 3: Block → Page → Way → Channel (순환)
	static unsigned char dieIndex = 0;
	targetDie = dieIndex;
	dieIndex = (dieIndex + 1) % USER_DIES;

#elif defined(ALLOCATION_POLICY_4)
	// Policy 4: Page → Block → Channel → Way (역순환)
	static unsigned char dieIndex = 0;
	if(dieIndex == 0)
		dieIndex = USER_DIES - 1;
	else
		dieIndex = dieIndex - 1;
	targetDie = dieIndex;
